...
class FullBridgeDesign(ConverterDesign):
    ...
    def normalize_cfg(cfg: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            'eff': p(cfg.get('input', {}).get('eff') or 0.95), 'cin_vrip': p(cfg.get('input', {}).get('cin_vrip') or 0.05*max(1.0, p(cfg.get('input', {}).get('vin_min') or 0))),
    ...
    def run_calculation(self, c: Dict[str, Any]) -> Dict[str, Any]:
        ...
        w1 = primary_turns(vin_min, d, bmax, ae, fsw); w2 = max(1, math.ceil(w1 / max(1e-9, n_ps)))
        ...
        losses = mosfet_loss_estimate(vds, i_rms, i_pk, fsw, {'rds_on_mohm': 30, 'tr_ns': 20, 'tf_ns': 40, 'qg_nC': 40, 'vgate_V': 10})
