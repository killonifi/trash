...
def compute_optocoupler(p: InputParams) -> Dict[str, Any]:
    ...
    # Controller FB range mapping check
    # Estimate collector voltage swing with nominal LED and with 'max' LED (vk near 2.0 V)
    i_led_nom = max((p.v_bias_zener - p.v_f_led - p.vk_work)/max(r_led,1e-9), 0.0)
    i_led_max = max((p.v_bias_zener - p.v_f_led - 2.0)/max(r_led,1e-9), 0.0)  # crude 'max' with deep conduction
    i_c_nom = p.ctr_min * i_led_nom
    i_c_max = p.ctr_min * i_led_max
    v_fb_high_ach = max(p.vce_sat, p.vdd - i_c_nom * p.r_pullup)   # LED small -> collector high
    v_fb_low_ach  = max(p.vce_sat, p.vdd - i_c_max * p.r_pullup)   # LED big  -> collector low
    ...
    if (v_fb_high_ach < p.vfb_max - 0.05) or (v_fb_low_ach > p.vfb_min + 0.05):
        warnings.append("Достижимый диапазон FB не покрывает диапазон контроллера: {:.2f}..{:.2f} В vs требуемых {:.2f}..{:.2f} В. Увеличьте CTR, уменьшите R_LED или R_pullup.".format(v_fb_low_ach, v_fb_high_ach, p.vfb_min, p.vfb_max))
    if p.ctr_min < 0.2:
    ...
    report_text = []
    report_text.append(_fmt_table(asdict(p), "Входные данные (Opto)"))
    report_text.append("")
    core_derived = {
            "R_upper_Ohm": r_up,
            "R_lower_Ohm": r_low,
            ...
            "R_LED_sel_Ohm": r_led,
            "V_k_work_V": p.vk_work,
            "I_LED_nom_A": i_led_nom,
            "G2_extra": g2,
            "f_opto_Hz": f_opto,
            "fpI_Hz": fpI,
            "Vfb_low_ach_V": v_fb_low_ach,
            "Vfb_high_ach_V": v_fb_high_ach,
        }
    report_text.append(_fmt_table(core_derived, "Производные параметры"))
    report_text.append("")
    report_text.append(_fmt_table({
            "R1_Ohm": r1_e24, "C2_F": c2_e24, "fz2_Hz": fz2_eff,
            "R2_Ohm": r2_e24, "C1_F": c1_e24, "fz1_Hz": fz1_eff,
            "R3_Ohm": r3_e24, "C3_F": c3_e24, "fp3_Hz": fp3_eff,
        }, "Сеть компенсации Type-3 (E24)"))
    report_text.append("")
    report_text.append(_fmt_table({
    ...
    out: Dict[str, Any] = {
        ...
        "derived": {
            ...
            "R_LED_sel_Ohm": r_led,
            "V_k_work_V": p.vk_work,
            "I_LED_nom_A": max((p.v_bias_zener - p.v_f_led - p.vk_work)/max(r_led,1e-9), 0.0),
            "G2_extra": g2,
            "f_opto_Hz": f_opto,
            "fpI_Hz": fpI,
            "Vfb_low_ach_V": v_fb_low_ach,
