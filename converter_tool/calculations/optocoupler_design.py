...
def design_type1(inp: Inputs) -> Results:
    ...
    notes = (
        f".: Type 1 (интегратор) :.",
        f"f_po = G(fc)·fc. C2 = CTR/(2π·f_po·RLED) − C_opто (если C2<0 → увеличьте fc или уменьшите RLED).",
        f"fc={fc:.1f} Hz, |G(fc)|={Glin:.3f}; f_opто={p['f_opто_hz']:.1f} Hz → f_opто/fc={(p['f_opто_hz']/fc):.2f}×. Рекомендовано f_opто ≥ 3…4·fc.",
...
# Делитель
    ...
    notes_local = (
        ...
        f"f_opто={p['f_opто_hz']:.1f} Hz ⇒ верхний fp ограничивайте ≤ 0.3…0.5·f_opто.",
...
def _autosolve_once(t: str, inp: Inputs, user: Dict[str, float], res: Results) -> (bool, Dict[str, float]):
    ...
    # Desired zener bias target (geometric mean within allowed band)
    Iz_lo = user.get("Iz_min", 2e-3)
    Iz_hi = user.get("Iz_max", 15e-3)
    if Iz_lo > 0 and Iz_hi > Iz_lo:
        user["Iz_min"] = (Iz_lo * Iz_hi) ** 0.5  # steer designs to mid-band Iz
...
def compute_optocoupler(p: InputParams) -> Dict[str, Any]:
    ...
    try:
        if p.plant_rhpz_hz and p.fc > p.plant_rhpz_hz/5.0:
            lines.append(f"Warning: fc={p.fc:.1f} Hz exceeds RHPZ/5={p.plant_rhpz_hz/5.0:.1f} Hz; stability margin may be poor.")
    except Exception:
        pass
