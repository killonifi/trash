...
def _compute_type2(p, r_led, r_up, r_low):
    """Type-2: 1 pole at origin (fpI), 1 zero (fz1), 1 HF pole (fp2)."""
    import math
    fc = max(p.fc, 1.0)
    fz = max(p.fz1, 1.0)
    fp2 = max(getattr(p, "fp2", 2.0*fc), 1.0)
    c1 = max(p.c1_nf*1e-9, 1e-12)      # zero
    c3 = max(p.c3_nf*1e-9, 1e-12)      # HF pole
    r2 = 1.0/(TWOPI * fz * c1)
    r3 = 1.0/(TWOPI * fp2 * c3)
    f_opto = 1.0/(TWOPI * p.r_pullup * (p.copto_nf*1e-9))
    g2 = (p.r_pullup * p.ctr_min) / max(r_led, 1e-9)
    ...
    g_const = g2 * g_opto * g_comp_const
    fpI = max(gc_target * fc / max(g_const, 1e-18), 1.0)
    c_int = c1
    r1 = 1.0/(TWOPI * fpI * c_int)
    ...
    g_total_db = 20.0*math.log10(max(g_total,1e-18))
    return {
        "type": "Type-2 (no fast lane)",
        "parts": { "R1_Ohm": r1_e, "R2_Ohm": r2_e, "R3_Ohm": r3_e, "C1_F": c1_e, "C3_F": c3_e },
        "freqs": { "fz_Hz": fz_e, "fpI_Hz": fpI_e, "fp2_Hz": fp2_e, "f_opto_Hz": f_opto_e },
        "gain_at_fc_dB": g_total_db,
    }
...
def compute_optocoupler(p: InputParams) -> Dict[str, Any]:
    ...
    lines.append(f"  TL431: Vref={p.v_ref:.3g} V, Idiv≈{p.i_div_uA:.0f} µA → Rupper≈{r_up:.0f} Ω, Rlower≈{r_low:.0f} Ω")
    ...
    out = {
        ...
        "network": network,
        "divider": {"Rupper_Ohm": r_up, "Rlower_Ohm": r_low},
