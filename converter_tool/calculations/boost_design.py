...
class BoostDesign(ConverterDesign):
    @staticmethod
    ...
    def run_calculation(self, cfg: Dict[str, Any]) -> Dict[str, Any]:
        ...
        d = 1.0 - vin / max(1e-9, vout); delta_i = cfg["delta_i"]
        L = (vin * d) / (delta_i * fsw); C = (iout * d) / (cfg["ripple_v"] * fsw)
        i_in = (vout * iout) / (cfg["eff"] * vin); Cin = i_in * (1 - d) / (cfg["cin_vrip"] * fsw)
        v_switch = vout
        i_rms, i_pk = estimate_switch_currents_boost(vin, vout, iout, d, delta_i, cfg["eff"])
        mosfets = recommend_mosfets(v_switch, i_rms, i_pk, fsw)
        losses = mosfet_loss_estimate(v_switch, i_rms, i_pk, fsw, {'rds_on_mohm': 12, 'tr_ns': 12, 'tf_ns': 12, 'qg_nC': 30, 'vgate_V': 10})
        return {
            ...
            "vds_max_V": round(v_switch, 2), "diode_vrrm_V": round(v_switch, 2),
