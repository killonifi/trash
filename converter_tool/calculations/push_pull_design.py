...
class PushPullDesign(ConverterDesign):
    ...
    def normalize_cfg(cfg: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            'eff': p(cfg.get('input', {}).get('eff') or 0.93), 'cin_vrip': p(cfg.get('input', {}).get('cin_vrip') or 0.05*max(1.0, p(cfg.get('input', {}).get('vin_min') or 0))),
    ...
    def run_calculation(self, c: Dict[str, Any]) -> Dict[str, Any]:
        ...
        n_ps = (2.0 * vin_min * d) / (vout + vd)
        w_half = primary_turns(vin_min/2.0, d, bmax, ae, fsw)
        w1_total = int(max(1, math.ceil(w_half)) * 2)
        w2_total = int(max(1, math.ceil(w_half / max(1e-9, n_ps))) * 2)
        vds = 2.0 * vin_max; pout = vout * iout
        delta_i = 0.35 * iout
        ...
        return {
            ...
            'primary_turns_total': w1_total, 'secondary_turns_total': w2_total,
