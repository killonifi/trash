...
def refine_with_core(fin: FlybackInput, geom: Geometry, core: CoreParameters,
    ...
    # - If only a gap is provided (Np/Ns omitted), choose Np to make Lm_actual≈Lm_target,
    #   while ensuring ΔB does not exceed Bmax (i.e., Np >= Np_min from Bmax).
    np_from_gap = None
    if overrides:
        try:
            ...
            pass
        # Derive Np from gap if user provided gap and did not override Np
        try:
        ...
        if (gap_in not in (None, "")) and ("np_turns" not in overrides or str(overrides.get("np_turns")).strip() == ""):
            try:
                ...
                # L = mu0 * Np^2 * Ae / gap  ->  Np = sqrt(L_target * gap / (mu0 * Ae))
                np_from_gap = int(math.ceil(max(1.0, (ini.lm_target_H * gap_val / (MU0 * Ae)) ** 0.5 )))
                # Respect ΔB constraint (Bmax): Np must be at least np_min
                np_turns = max(np_min, np_from_gap)
            except Exception:
                pass
