...
def refine_with_core(fin: FlybackInput, geom: Geometry, core: CoreParameters,
    ...
    # Apply optional overrides for transformer turns
    # New behavior: if only gap is provided (Np/Ns omitted),
    # derive Np to meet Lm_target, but enforce ΔB constraint (Np >= np_min from Bmax).
    np_from_gap = None
    np_reason = "bmax_limit"
    if overrides:
        try:
            np_override = overrides.get("np_turns")
            if np_override not in (None, ""):
                np_turns = int(max(1, float(np_override)))
                np_reason = "user_np"
        except Exception:
            pass
        try:
            ...
            gap_in = None
        if (gap_in not in (None, "")) and ("np_turns" not in overrides or str(overrides.get("np_turns")).strip() == ""):
            try:
                gap_val = float(gap_in)
                np_from_gap = int(math.ceil(max(1.0, (ini.lm_target_H * gap_val / (MU0 * Ae)) ** 0.5 )))
                if np_from_gap >= np_min:
                    np_turns = np_from_gap
                    np_reason = "from_gap"
                else:
                    np_turns = np_min
                    np_reason = "bmax_limit"
    ...
    # diagnostics
    mismatch_Lm_ratio = lm_actual / max(1e-18, ini.lm_target_H)
    Np_min_Bmax = np_min
    try:
        gap_req_for_Lm_target_m = MU0 * (np_turns**2) * Ae / max(ini.lm_target_H, 1e-18)
    except Exception:
        gap_req_for_Lm_target_m = None
    try:
        gap_in_dbg = (overrides or {}).get("gap_m")
        Np_req_for_Lm_target = int(math.ceil(max(1.0, (ini.lm_target_H*float(gap_in_dbg)/ (MU0*Ae))**0.5 ))) if gap_in_dbg not in (None,"") else None
    except Exception:
        Np_req_for_Lm_target = None
    
    notes = dict(mismatch_Lm_ratio=mismatch_Lm_ratio, np_min_Bmax=Np_min_Bmax, gap_req_for_Lm_target_m=gap_req_for_Lm_target_m, np_req_for_Lm_target=Np_req_for_Lm_target, np_selection=np_reason, np_min_from_Bmax=np_turns, period_s=period)
...
def sweep_k(fin: FlybackInput, outputs: List[OutputSpec], geom: Geometry, core: CoreParameters,
            ...
            force_dcm: bool=False, overrides: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    main_name = fin.main_output or outputs[0].name
    grid = []
    best = None
    for D in [dmin + i*dstep for i in range(int((dmax-dmin)/dstep)+1)]:
        ...
        ref = refine_with_core(fin, geom, core, ini, outputs, rcd=rcd, stein=stein, mosfet=mosfet,
                               overrides=overrides, force_dcm=force_dcm)
...
def run(cfg: Dict[str, Any], corelib: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    # ##TRANS_OVER: optional transformer overrides from GUI
    trans_over = cfg.get("transformer_overrides", {}) or {}
    ...
    if "core" in cfg and "geometry" in cfg:
        ...
        if cfg.get("k_optimize"):
            ...
            sweep = sweep_k(fin, outs, geom, core, rcd=rcd, stein=stein, mosfet=mosfet, criterion=crit,
                            ...
                            overrides=trans_over, force_dcm=fin.force_dcm)
            ...
            try:
                if res["refined"].get("notes",{}).get("mismatch_Lm_ratio",1.0) > 1.1:
                    res.setdefault("warnings", []).append("Lm_actual значительно больше Lm_target; увеличьте зазор или Ae, либо уменьшите Bmax/частоту.")
            except Exception:
                pass
