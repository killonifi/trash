...
class App(tk.Tk):
    ...
    def build_optocoupler_tab(self):
        ...
        self.opto_vars = {k: tk.StringVar() for k in [
            ...
            "i_div_uA","v_bias_zener","i_bias_mA","fc","gc_db","fz1","fz2","fp3","c1_nf","c2_nf","c3_nf", "vk_work", "vfb_min", "vfb_max", "opto_model"
        ]}
        # defaults
        ...
        add_row(left, r, "Fsw, Hz", "f_sw", "Частота ШИМ контроллера"); r+=1
...
info.insert("1.0",
"""ЗАЧЕМ ЭТО ВСЁ
— Оптопара медленная: у неё есть полюс f_opto. Мы формуем петлю так, чтобы не «упереться» фазой в этот потолок, и одновременно получить быстрый и чистый переходный.
...
БАЗОВЫЕ ЧАСТОТЫ
— f_opto = 1/(2π·Rpullup·(Copto + C2)) — полюс оптопары (предел скорости). Чем больше Rpullup и (Copto+C2), тем ниже f_opto.
— fc — частота пересечения петли. Это «скорость» системы. Держите fc заметно ниже f_opto и не выше fsw/10.
...
1) Посчитайте f_opto с наброском C2 (например 4.7 нФ) и вашим Rpullup, Copto.
2) Выберите fc = min(0.25·f_opto, fsw/10). Если слишком медленно — боритесь за более высокий f_opto (уменьшите Rpullup, C2; возьмите оптопару с меньшим Copto).
...
— Типичные стартовые: C1=10 нФ, C2=4.7 нФ, C3=1.0 нФ (E24). Помните: C2 стоит параллельно Copto → понижает f_opto, не раздувайте её без нужды.
...
— Если окно не перекрывается — увеличьте CTR, уменьшите RLED и/или Rpullup (следите за f_opto), либо поднимите Vbias.
...
ОПТОПАРЫ (БИБЛИОТЕКА)
— Ключевые параметры: CTR(min) в вашей рабочей точке и Copto (влияет на f_opto).
— Быстрые оптопары имеют меньшую Copto и/или лучшую частотную характеристику → выше f_opto и лучший фазовый запас.
...
2) Оценил f_opto → взял fc≈0.25·f_opto (и ≤ fsw/10).
3) Поставил fz1≈0.3·fc, fz2≈0.9·fc, fp3≈3·fc. C1=10 нФ, C2=4.7 нФ, C3=1 нФ → получил R1,R2; R3 подбирает расчёт по Gc(fc).
4) Проверил перекрытие FB‑окна и предупреждения. Если не сходится — CTR↑, RLED↓, Rpullup↓ (проверяя f_opto), Vbias↑.
""")
