...
OPTO_INFO_TEXT = r"""
ОБЩЕЕ
— Держите fc ≤ min(fsw/10, 0.25·f_opто). f_opто = 1/(2π·Rpullup·(C2_fb + C_opto)).
— Рекомендуемые стартовые: C1=10 нФ, C2=4.7 нФ, C3=1 нФ; корректируйте, чтобы резисторы были 2 к…200 кΩ.

TYPE‑1 (FAST‑LANE: БЕЗ Vz)
— Структура: интегратор (полюс в нуле). Без дополнительных нулей/полюсов.
— Вы выбираете fc. Программа подбирает Rint из |G(fc)|.
— Контроль: f_opто должен быть как минимум в 3–4 раза выше fc.

TYPE‑2 / TYPE‑2_FAST (1 НУЛЬ + 1 ПОЛЮС)
— Структура: интегратор + нуль fz1 (R2‑C1) + ВЧ полюс fp2 (R3‑C3).
— Как ставить частоты по ON Semi:
   • fz1 ≈ 0.3·fc (поднятие фазы к пересечению);
   • fp2 ≈ 2…4·fc (срез усиления выше fc, чуть ниже f_opто, но не ближе fc/1.5).
— Алгоритм: задайте fz1, fp2 и C1,C3 → программа выдаст R2, R3 и Rint.

TYPE‑3 (2 НУЛЯ + 2 ПОЛЮСА)
— Структура: интегратор + два нуля (fz1, fz2) + ВЧ полюс (fp3). Интеграторный полюс (fpI) подбирается по |G(fc)| — это второй полюс.
— Как ставить частоты по ON Semi:
   • fz1 ≈ 0.2…0.4·fc, fz2 ≈ 0.7…1.0·fc;
   • fp3 ≈ 2…4·fc; fpI рассчитывается программой из цели |G(fc)|.
— Алгоритм: задайте fz1, fz2, fp3 и C1,C2,C3 → программа выдаст R2,R3 и Rint.

ОКНО VFB КОНТРОЛЛЕРА И СМЕЩЕНИЕ
— Проверьте перекрытие окна FB (VFB_min…VFB_max) током коллектора (формулы TI).
— При no‑fast‑lane используйте Vz и рассчитайте Rbias для нужного тока TL431.
"""
