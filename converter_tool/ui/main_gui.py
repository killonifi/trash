...
class OptoLibraryWindow(tk.Toplevel):
    def __init__(self, master, lib_path):
        ...
        self.tree = ttk.Treeview(self, columns=("model","vendor","ctr_min","copto_nf","vce_sat","notes"), show="headings")
        for c, w in zip(("model","vendor","ctr_min","copto_nf","vce_sat","notes"), (120,120,80,80,80,160)):
            self.tree.heading(c, text=c)
            self.tree.column(c, width=w, anchor="w")
        self.tree.pack(fill="both", expand=True)
        btns = ttk.Frame(self); btns.pack(fill="x")
        ttk.Button(btns, text="Apply to Optocoupler Tab", command=self.apply).pack(side="left", padx=4, pady=4)
        ttk.Button(btns, text="Close", command=self.destroy).pack(side="right", padx=4, pady=4)
        try:
        ...
        self.data = data
        for item in data:
            self.tree.insert("", "end", values=(item.get("model",""), item.get("vendor",""), item.get("ctr_min",""),
                                                item.get("copto_nf",""), item.get("vce_sat",""), item.get("notes","")))
    def apply(self):
        sel = self.tree.selection()
        if not sel:
            return
        vals = self.tree.item(sel[0], "values")
        model, vendor, ctr_min, copto_nf, vce_sat, notes = vals
        # write to master's opto vars if present
        try:
...
class App(tk.Tk):
    ...
    def compute(self):
        """Run full converter calculation and render into Results tab."""
        try:
    ...
    def compute_optocoupler(self, cfg_norm=None):
        try:
            if cfg_norm is None:
                cfg = self.collect_cfg()
                cfg_norm = self.design.normalize_cfg(cfg)
            gvars = {k: self.opto_vars[k].get().strip() for k in self.opto_vars}
            def fget(k, default):
                try: return float(gvars.get(k) or default)
            ...
            if not text:
                ...
                if report.get("warnings"):
                    lines.append("\n[WARNINGS]"); lines += ["! "+w for w in report["warnings"]]
                text = "\n".join(lines)
            self.opto_text.delete("1.0", "end")
            self.opto_text.insert("1.0", text)
        except Exception as e:
            self.opto_text.delete("1.0", "end")
    ...
    def run_sweep(self):
        if not hasattr(self.design, "sweep_k"):
            messagebox.showinfo("K-optimizer", "This design does not support K sweep")
            return
        try:
            cfg = self.collect_cfg()
            cfg_norm = self.design.normalize_cfg(cfg)
