...
def on_comp_type_changed(self, event=None):
        ...
        # Show only fields relevant to each type
        visible = set()
        # always visible: c1 for all types
        visible.update({"c1_nf"})
        if t == "type1":
            pass  # only C1 needed
        elif t == "type2":
            visible.update({"fz1"})  # R2 via fz1
        elif t == "type3":
            visible.update({"fz1", "fz2", "fp3", "c2_nf", "c3_nf", "v_bias_zener"})
        # Apply visibility
        for key, frm in self.opto_field_frames.items():
            if key in visible:
                ...
                frm.grid_remove()
        # Picture
        img_name = {"type1":"Optocoupler_type1.png","type2":"Optocoupler_type2.png","type3":"Optocoupler_type3.png"}.get(t, "Optocoupler_type1.png")
