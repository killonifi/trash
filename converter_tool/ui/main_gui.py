...
class App(tk.Tk):
    ...
    def build_optocoupler_tab(self):
        ...
        def _auto_cb(*_):
            return  # disabled: do not auto-disable Advanced
            try:
...
# --- Compensation type selector ---
    ...
    def compute_optocoupler(self, cfg_norm=None):
        try:
            ...
            # --- Choose fc (cap only in auto-tune) ---
            fc_req = fget("fc", cfg_norm["input"].get("fsw", 100000.0)/10.0)
            auto_on = bool(int(g.get("auto_tune","1") or "0"))
            if auto_on:
                # Try to cap by plant RHPZ only in auto mode
                rhpz_main = None
                try:
                    res = getattr(self, "last_result", None)
                    if res and "refined" in res:
                        main_name = res.get("input",{}).get("main_output") or res["outputs"][0]["name"]
                        rhpz_main = res["refined"].get("rhpz_each_Hz",{}).get(main_name)
                        if rhpz_main and rhpz_main>0:
                            fc_req = min(fc_req, rhpz_main/5.0)
                except Exception:
                    pass
